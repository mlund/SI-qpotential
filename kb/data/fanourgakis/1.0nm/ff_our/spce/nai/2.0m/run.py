...
system = ff.createSystem(
    ...
    nonbondedCutoff=1.000000*nanometers, constraints=AllBonds, rigidWater=True)

nonbonded = findForce(system, CustomNonbondedForce)
nonbonded.addGlobalParameter( 'Rc', 1.0*nanometers )           # 'Rc' can now be used in energy function
nonbonded.addGlobalParameter( 'f', elec_to_kJmol ) # 'lB' bjerrum length in nm and kJ/mol

nonbonded.setEnergyFunction(
    'f * charge1 * charge2 * 1/r * ( 1 - 1.75*r/Rc + 5.25*(r/Rc)^5 - 7*(r/Rc)^6 + 2.5*(r/Rc)^7 )'     ' + 4 * epsilon * ( (sigma/r)^12 - (sigma/r)^6 )'     ' ; sigma = 0.5 * (sigma1+sigma2); epsilon = sqrt( epsilon1*epsilon2 )'
)
